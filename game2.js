...
const createWorkElement = (work, index) => {
    const workItem = document.createElement("div");
    workItem.className = "work-item";

    const workButton = document.createElement("button");
    workButton.type = "button";
    workButton.id = `button${index}`;
    workButton.textContent = work.name;
    workButton.addEventListener("click", () => workClick(index));

    const infoElem = document.createElement("span");
    infoElem.className = "work-info";
    infoElem.id = `info${index}`;

    const autoButton = document.createElement("button");
    autoButton.type = "button";
    autoButton.className = "action-button";
    autoButton.id = `auto${index}`;
    autoButton.addEventListener("click", () => buyAuto(index));

    const buyButton = document.createElement("button");
    buyButton.type = "button";
    buyButton.className = "action-button";
    buyButton.id = `buy${index}`;
    buyButton.addEventListener("click", () => buyWork(index));

    workItem.append(workButton, infoElem, autoButton, buyButton);
    return workItem;
}

const renderWorkItems = () => {
    const workButtons = document.getElementById("work-buttons");
    if (!workButtons) {
        return;
    }

    workButtons.replaceChildren(...gameStates.work.map(createWorkElement));
}
